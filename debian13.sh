...
echo 'zh_CN.UTF-8 UTF-8' >> /etc/locale.gen && locale-gen && echo 'LANG=zh_CN.UTF-8' > /etc/default/locale && export LANG=zh_CN.UTF-8
ln -fs /usr/share/zoneinfo/Asia/Shanghai /etc/localtime && echo "Asia/Shanghai" > /etc/timezone && dpkg-reconfigure -f noninteractive tzdata
